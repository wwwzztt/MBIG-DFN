...
class fft_bench_complex_mlp(nn.Module):
    def __init__(self, dim, dw=1, norm='backward', act_method=nn.ReLU, window_size=None, bias=False,fft_window=False):
        super(fft_bench_complex_mlp, self).__init__()
        self.fft_window=fft_window
        self.act_fft = act_method()
        self.window_size = window_size  # [64, 32, 16, -1]
        # dim = out_channel
        hid_dim = dim * dw
        # print(dim, hid_dim)
        ...
        init.kaiming_uniform_(self.complex_weight1_real, a=math.sqrt(16))  # kaiming初始化
    ...
    def forward(self, x):
        _, _, H, W = x.shape
        if self.window_size is not None and (H != self.window_size or W != self.window_size) and self.window_size>=0 and self.fft_window is True:
            x, batch_list = window_partitionx(x, self.window_size)  # 切割为窗口
        y = torch.fft.rfft2(x, norm=self.norm)  # fft
        dim = 1
        weight1 = torch.complex(self.complex_weight1_real, self.complex_weight1_imag)  # 实部与虚部组成卷积权重
